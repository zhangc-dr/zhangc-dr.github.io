...
class NonGroupNOMARegion:
    ...
    def compute_render(self, batch_size1=100, batch_size2=200, save_dir='data', img_file='img/ng_noma.png'):
        ...
        for start1 in range(0, total_v1, batch_size1):
            ...
            filepath = os.path.join(save_dir, f'ngnoma_batch_{start1}.csv')
                ...
                for v1 in batch_v1:
                    for v2 in batch_v2:
                        for a in self.alpha:
                            ...
                            R2_direct = np.log2(1 + p2 * h2v2 / (p1 * h2v1 + self.sigma_2))
                            R2_cross = np.log2(1 + p2 * h1v2 / (p1 * h1v1 + self.sigma_2))
                            R2 = min(R2_direct, R2_cross)
        ...
        # 合并所有批次 CSV
        files = glob.glob(os.path.join(save_dir, 'ngnoma_batch_*.csv'))
        all_points = []
