...
\( x = \sqrt{\alpha_c P_t}\, \mathbf{w}_c s_c + \sum_k \sqrt{\alpha_k P_t}\, \mathbf{w}_k s_k \),
where \(P_t\) is the transmit power, \(\mathbf{w}_c,\mathbf{w}_k\) are the precoding weights for \(s_c,s_k\),
