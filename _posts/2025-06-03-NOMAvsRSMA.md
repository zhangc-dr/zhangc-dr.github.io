...
# 1 Papers
## Paper1 : What Role can NOMA Play in Massive MIMO? -  Prof. Emil 
...
## Paper 2: [On Performance of Downlink THz-Based Rate-Splitting Multiple-Access (RSMA): Is it Always Better Than NOMA?](https://arxiv.org/abs/2305.07361) - Thai-Hoc Vu
### a. 文章所要解决的核心问题
...
#### b.2 RSMA发射信号的定义
The transmit signal is $x = \sqrt{\alpha_c P_t}\, \mathbf{w}_c s_c + \sum_k \sqrt{\alpha_k P_t}\, \mathbf{w}_k s_k$, 
where $P_t$ is the transmit power, $\mathbf{w}_c, \mathbf{w}_k$ are the precoding weights, 
with $\|\mathbf{w}_c\|^2 = \|\mathbf{w}_k\|^2 = 1$, 
and $\alpha_c, \alpha_k$ denote the power allocation coefficients with $\alpha_c + \sum_k \alpha_k = 1$.
...
在 RSMA 中，由于引入了新的待优化变量 $ \mathbf{w}_c $，使得问题变得更加复杂。要想完全消除干扰，在 $\gamma_c^k$ 中需要满足
...
而 $ \mathbf{g}_k^H \mathbf{w}_k = \left|\mathbf{g}_k \right| $ 和 $ \mathbf{g}_k^H \mathbf{w}_k = 0 $，以及 $ \mathbf{g}_k^H \mathbf{w}_c = 0 $ 和 $ \mathbf{g}_k^H \mathbf{w}_c = \left| \mathbf{g}_k \right| $ 是矛盾的，因此 RSMA 中不可能完全消除所有干扰。  
